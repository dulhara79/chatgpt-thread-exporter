...
<main class="document">
  ...
  async function fetchDocxImageAssets(turns) {
    ...
    for (const turn of turns) {
      const messages = [turn.question, ...(turn.answers || [])];
      for (const message of messages) {
        ...
        while ((match = imageRe.exec(source))) {
          ...
          const src = match[2];
          if (!seen.has(src) && shouldIncludeImage({ src, alt, width: 800, height: 500 })) seen.set(src, { src, alt });
        }
      }
    }
    const assets = [];
    for (const item of seen.values()) {
      try {
        ...
        assets.push({ src: item.src, alt: item.alt, bytes, ext, width, height, name: `image${assets.length + 1}.${ext}` });
      } catch {}
    }
    return assets;
  }
  ...
  async function createDocxBlob(data, turns, options = {}) {
    ...
    function imageDrawing(token) {
      ...
      const maxCx = 5.8 * 914400;
      const cx = Math.round(maxCx);
...
)) tokens.push({ type: 'math', text: value.slice(1, -1).trim(), display: false });
